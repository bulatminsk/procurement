...
<html>
    <head>
        ...
        <title><fmt:message key="title.page.listTenderByCommittee" bundle="${rb}"/></title>
    </head>
    <body>
        <c:import url="/jsp/structure/header.jsp"/>
        <c:set var="lastPage" value="/jsp/tender/list_tender_committee.jsp" scope="session"/>   
        <div class="container">
            <br>
            <br>
            <br>
            <h4 class="text-center"><fmt:message key="tenderByCommittee.table.name" bundle="${rb}"/>, ${user.login}</h4>
            ...
            <table class="table table-bordered">
                ...
                <tbody>
                    <c:if test="${empty committeeMemberTenders}">
                    <h4 class="text-center">
                        <fmt:message key="message.noTenders" bundle="${rb}"/>
                    </h4>
                </c:if>
                <c:forEach items="${committeeMemberTenders}" var="tender">
                    <tr>
